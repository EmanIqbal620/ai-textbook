...
def check_system():
    ...
    try:
        ...
        print(f"   [SUCCESS] Qdrant connected - {count.count} documents in collection")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   [SUCCESS] Backend healthy - Status: {health_data.get('status', 'unknown')}")
        else:
            print(f"   [ERROR] Backend error - Status: {response.status_code}")
    except requests.exceptions.ConnectionError:
        print("   [ERROR] Backend not running - Start with: cd backend && python server.py")
    except Exception as e:
    ...
    if os.path.exists(docs_path):
        ...
        print(f"   [SUCCESS] Textbook folder exists - {len(md_files)} markdown files found")
    else:
        print("   [ERROR] Textbook folder not found")
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print("   [SUCCESS] Query successful - Response received")
            print(f"   Response preview: {data['response'][:100]}...")
        else:
            print(f"   [ERROR] Query failed - Status: {response.status_code}")
    except requests.exceptions.ConnectionError:
        print("   [ERROR] Cannot connect - Backend may not be running")
    except Exception as e:
        print(f"   [ERROR] Query error: {e}")

    print("\n" + "=" * 50)
    print("System Status Summary:")
    print("[SUCCESS] Textbook content ingested into Qdrant")
    print("[SUCCESS] RAG system ready for queries")
    print("[SUCCESS] Backend API connection available")
    print("[SUCCESS] Ready for student interactions")
